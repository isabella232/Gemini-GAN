...
class GeneratorSRGANSegImg(nn.Module):
    def __init__(self,in_channels,out_channels,scale_factor):
        ...
        block8_img = [UpsampleBLockSRGAN(64, 64),UpsampleBLockSRGAN(64, 64)]
        self.up = nn.Sequential(*block8_img)
        self.block_img = nn.Conv2d(64, out_channels, kernel_size=3, padding=1)
        self.block_seg = nn.Conv2d(64, 4*out_channels, kernel_size=3, padding=1)
...
class GeneratorSRGANSegImgDouble(nn.Module):
    def __init__(self,in_channels,out_channels,scale_factor):
        ...
        block8_img = [UpsampleBLockSRGAN(64, 64),UpsampleBLockSRGAN(64, 64)]
        block8_seg = [UpsampleBLockSRGAN(64, 64),UpsampleBLockSRGAN(64, 64)]
...
class GeneratorSRGANSeg(nn.Module):
    ...
    def forward(self, x):
        ...
        up_data = self.up(block1+block12)
        logits  = self.block_seg(up_data).view(1, 4, img.size(1), img.size(2), img.size(3))
        return logits
...
class GeneratorSRGANImg(nn.Module):
    ...
    def forward(self, x):
        ...
        block12 = self.block12(block11)
        ##############################
        up_data = self.up(block12+block1)
        out_img = self.block_img(up_data)
        return out_img
