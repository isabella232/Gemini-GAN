...
class GeneratorSRGANSeg(nn.Module):
    def __init__(self,in_channels,out_channels,scale_factor):
        upsample_block_num = int(math.log(scale_factor, 2))

        super(GeneratorSRGANSeg, self).__init__()        
   
        self.inc    = DoubleConv(in_channels, 16)
        self.down1  = Down(16, 32)
        self.block1 = Down(32, 64)
        ...
        block8_seg = [UpsampleBLockSRGAN(64, 32),UpsampleBLockSRGAN(32, 16)]
        self.up = nn.Sequential(*block8_seg)
        self.block_seg = nn.Conv2d(16, 4*out_channels, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        up_data = self.up(block6+block1)

        logits  = self.block_seg(up_data).view(1, 4, img.size(1), img.size(2), img.size(3))
...
class GeneratorSRGANSegImg(nn.Module):
    def __init__(self,in_channels,out_channels,scale_factor):
        upsample_block_num = int(math.log(scale_factor, 2))

        super(GeneratorSRGANSegImg, self).__init__()        
        ...
        '''
        self.block7  = ResidualBlockSRGAN(64)
        self.block8  = ResidualBlockSRGAN(64)
        self.block9  = ResidualBlockSRGAN(64)
        self.block10 = ResidualBlockSRGAN(64)
        self.block11 = ResidualBlockSRGAN(64)
        self.block12 = ResidualBlockSRGAN(64)
        ###################################
        self.block13  = ResidualBlockSRGAN(64)
        self.block14  = ResidualBlockSRGAN(64)
        self.block15  = ResidualBlockSRGAN(64)
        self.block16  = ResidualBlockSRGAN(64)
        self.block17  = ResidualBlockSRGAN(64)
        self.block18  = ResidualBlockSRGAN(64)
        '''


        block8_img = [UpsampleBLockSRGAN(64, 32),UpsampleBLockSRGAN(32, 16)]
        self.up = nn.Sequential(*block8_img)
        self.block_img = nn.Conv2d(16, out_channels, kernel_size=3, padding=1)
        self.block_seg = nn.Conv2d(16, 4*out_channels, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        logits  = self.block_seg(up_data).view(1, 4, img.size(1), img.size(2), img.size(3))
        out_img = self.block_img(up_data)
        return out_img,logits



class GeneratorSRGANSegImgDouble(nn.Module):
    def __init__(self,in_channels,out_channels,scale_factor):
        upsample_block_num = int(math.log(scale_factor, 2))

        super(GeneratorSRGANSegImgDouble, self).__init__()        
        ...
        block8_img = [UpsampleBLockSRGAN(64, 32),UpsampleBLockSRGAN(32, 16)]
        block8_seg = [UpsampleBLockSRGAN(64, 32),UpsampleBLockSRGAN(32, 16)]
        self.up_img = nn.Sequential(*block8_img)
        self.up_seg = nn.Sequential(*block8_seg)
        self.block_img = nn.Conv2d(16, out_channels, kernel_size=3, padding=1)
        self.block_seg = nn.Conv2d(16, 4*out_channels, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        up_img = self.up_img(block6+block1)
        up_seg = self.up_seg(block6+block1)

        logits  = self.block_seg(up_seg).view(1, 4, img.size(1), img.size(2), img.size(3))
        out_img = self.block_img(up_img)
        return out_img,logits


class GeneratorSRGANImg(nn.Module):
    def __init__(self,in_channels,out_channels,scale_factor):
        upsample_block_num = int(math.log(scale_factor, 2))

        super(GeneratorSRGANImg, self).__init__()        
...
class UpsampleBLockSRGAN(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(UpsampleBLockSRGAN, self).__init__()
        #self.conv = nn.Conv2d(in_channels, in_channels * up_scale ** 2, kernel_size=3, padding=1)
        #self.pixel_shuffle = nn.PixelShuffle(up_scale)
        self.upsempling =  nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        self.conv = DoubleConv(in_channels, out_channels)
        #self.prelu = nn.PReLU()

    def forward(self, x):
        #x = self.conv(x)
        x = self.conv(self.upsempling(x))
        #x = self.prelu(x)
        return x
...
class DiscriminatorOriginal(nn.Module):
    def __init__(self,in_channels,k):
        ...
        self.net = nn.Sequential(
            nn.Conv2d(k*in_channels, 32, kernel_size=4, padding=1),
            nn.LeakyReLU(0.2),
...
class Discriminator(nn.Module):
    def __init__(self,in_channels,k):
        ...
        self.net = nn.Sequential(
            nn.Conv2d(k*in_channels, 64, kernel_size=3, padding=1),
            nn.LeakyReLU(0.2),
...
class discriminator_srgan(nn.Module):
    def __init__(self,in_channels):
        ...
        self.net = nn.Sequential(
            nn.Conv2d(2*in_channels, 32, kernel_size=4, padding=1),
            nn.LeakyReLU(0.2),
...
'''
class DeepGeneratorSRGANSegImg(nn.Module):
    def __init__(self,in_channels,out_channels,scale_factor):
        upsample_block_num = int(math.log(scale_factor, 2))

        super(DeepGeneratorSRGANSegImg, self).__init__()        
        ...
        self.block11 = ResidualBlockSRGAN(64)
        ###################################
        block8_img = [UpsampleBLockSRGAN(64, 2) for _ in range(upsample_block_num)]
        self.up = nn.Sequential(*block8_img)
        self.block_img = nn.Conv2d(64, out_channels, kernel_size=9, padding=4)
        self.block_seg = nn.Conv2d(64, 4*out_channels, kernel_size=9, padding=4)
    ...
    def forward(self, x):
        ...
        block9  = self.block9(block8)
        block10 = self.block10(block9)
        block11 = self.block10(block10)
        ############################
        up_data = self.up(block11)
        logits  = self.block_seg(up_data).view(1, 4, img.size(1), img.size(2), img.size(3))
        out_img  = self.block_img(up_data)
        return out_img,logits
'''
...
class ResidualForward(nn.Module):
    def __init__(self, channels):
        super(ResidualForward, self).__init__()
        self.conv1 = nn.Conv2d(channels, channels, kernel_size=3, padding=1)
        self.bn1 = nn.BatchNorm2d(channels)
        self.prelu = nn.PReLU()
        self.conv2 = nn.Conv2d(channels, channels, kernel_size=3, padding=1)
...
class ResidualBackward(nn.Module):
    def __init__(self, channels):
        super(ResidualBackward, self).__init__()
        self.conv1 = nn.Conv2d(channels, channels, kernel_size=3, padding=1)
        self.bn1 = nn.BatchNorm2d(channels)
        self.prelu = nn.PReLU()
        self.conv2 = nn.Conv2d(channels, channels, kernel_size=3, padding=1)
    ...
    def forward(self, x_prev,x_next):
        residual = self.conv1(x_next)
        residual = self.bn1(residual)
        residual = self.prelu(residual)
        residual = self.conv2(residual)
        residual = self.bn2(residual)
        return  residual - x_prev

class usuperes(nn.Module):
    def __init__(self,in_channels,out_channels,scale_factor):
        upsample_block_num = int(math.log(scale_factor, 2))

        super(usuperes, self).__init__()
        ...
        ###################################
        self.block2_f = ResidualForward(64)
        self.block3_f = ResidualForward(64)
        self.block4_f = ResidualForward(64)
        self.block5_f = ResidualForward(64)
        self.block6_f = ResidualForward(64)
        ###################################
        self.block2_b = ResidualBackward(64)
        self.block3_b = ResidualBackward(64)
        self.block4_b = ResidualBackward(64)
        self.block5_b = ResidualBackward(64)
        self.block6_b = ResidualBackward(64)
        ###################################

        block8_img     = [UpsampleBLockSRGAN(64, 2) for _ in range(upsample_block_num)]
        self.up_seg    = nn.Sequential(*block8_img)
        self.up_img    = nn.Sequential(*block8_img)

        self.out_img = nn.Conv2d(64, out_channels, kernel_size=3, padding=1)
        self.out_seg = nn.Conv2d(64, 4*out_channels, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        block2_f   = self.block2_f(block1)
        block2_b   = self.block2_b(block1,block2_f)

        block3_f   = self.block3_f(block2_b)
        block3_b   = self.block3_b(block2_b,block3_f)
 

        block4_f   = self.block4_f(block3_b)
        block4_b   = self.block4_b(block3_b,block4_f)


        block5_f   = self.block5_f(block4_b)
        block5_b   = self.block5_b(block4_b,block5_f)


        block6_f   = self.block6_f(block5_b)
        block6_b   = self.block6_b(block5_b,block6_f)
    
        up_data_seg = self.up_seg(block6_b)
        up_data_img = self.up_img(block6_b)
    
        logits      = self.out_seg(up_data_seg).view(1, 4, img.size(1), img.size(2), img.size(3))
        out_img     = self.out_img(up_data_img)
        return out_img,logits
