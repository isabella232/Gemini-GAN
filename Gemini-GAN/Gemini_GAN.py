...
import torch
from   model import GeneratorUnetSegImg,Discriminator,DiscriminatorOriginal
import numpy as np 
...
def load(path_model):
    generator     = GeneratorUnetSegImg(in_channels,out_channels)
...
netD = Discriminator(in_channels)
netD = netD.cuda()
today     =  date.today()
...
for epoch in range(n_epochs):
    ...
    for batch_idx, batch_train in tqdm(enumerate(training_data_loader,\
        ...
        if flag_GAN:
            netG,g_loss,\
            output_hr,output_seg_hr = trainGAN(generator,input_img,input_gt,gt_seg)
        else:
            netG,g_loss,\
            output_hr,output_seg_hr = train(generator,input_img,input_gt,gt_seg)
    ...
    if epoch > 0 and epoch % every_valid==0:
        ...
        final_psnr_val_ed,final_psnr_val_es,\
        final_ssim_val_ed,final_ssim_val_es = valid_seg(generator,pathData,epoch_save)
